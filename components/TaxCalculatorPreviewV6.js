...
const cards = {
  pph21: {
    ...
    text: 'TER berlaku untuk pegawai tetap pada masa selain masa pajak terakhir. Kode objek bukti potong mengikuti PER-11/PJ/2025.',
    links: [
  ...
  unifikasi: {
    ...
    text: 'Objek dikelompokkan berdasarkan jenis PPh utama agar lebih mudah dipilih dan diperiksa.',
    links: [
...
const unifikasiGroups = {
  p23: {
    label: 'PPh Pasal 23',
    items: [
      ['p23j', 'Jasa teknik, manajemen, konsultan, dan jasa lain', 2],
      ['p23s', 'Sewa harta selain tanah dan/atau bangunan', 2],
      ['p23b', 'Bunga, royalti, hadiah/penghargaan tertentu', 15]
    ]
  },
  p42: {
...
function P21() {
  ...
  const [prior, setPrior] = useState('2200000');
  ...
  const annual = useMemo(() => calculateAnnualPph21ByMethod({ annualGross, pensionContribution: pension, ptkpStatus: status, monthsWorked: months, priorWithheld: prior, method }), [annualGross, pension, status, months, prior, method]);
...
function UMKM() {
  ...
  const [before, setBefore] = useState('450000000');
  const [month, setMonth] = useState('100000000');
  const [previousYear, setPreviousYear] = useState('1000000000');
...
function UNI() {
  ...
  const [objectId, setObjectId] = useState('p23j');
  const [amount, setAmount] = useState('100000000');
  ...
  return <div className="calculator-grid"><section className="panel form-panel">
    ...
    <label className="field"><span>Objek PPh Unifikasi</span><select value={objectId} onChange={(e) => setObjectId(e.target.value)}>{group.items.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select><small>Daftar objek otomatis mengikuti jenis PPh utama yang dipilih.</small></label>
...
function CorpTax() {
  ...
  const [manualRate, setManualRate] = useState('22');
  const [revenue, setRevenue] = useState('10000000000');
  const [commercialProfit, setCommercialProfit] = useState('1200000000');
  const [corrections, setCorrections] = useState([
    { id: 1, name: 'Biaya yang tidak dapat dikurangkan', type: 'positive', amount: '100000000' },
    { id: 2, name: 'Penghasilan dikenai PPh Final', type: 'negative', amount: '50000000' },
    { id: 3, name: 'Penghasilan bukan objek pajak', type: 'negative', amount: '0' }
  ]);
  const [assets, setAssets] = useState([
    { id: 1, name: 'Laptop & komputer', kind: 'tangible', cost: '24000000', month: '1', year: '2026', group: 'group1', method: 'sl', commercial: '8000000' },
    { id: 2, name: 'Software khusus / lisensi', kind: 'intangible', cost: '120000000', month: '1', year: '2025', group: 'group2', method: 'sl', commercial: '24000000' }
  ]);
